clc
clear
close all
tic

%%
TempIctalSegmentNum = load('..\..\..\..\03_SeizureDetection\m_FrontalIctalSegmentNum.mat');
TempIctalSegmentNum = struct2cell(TempIctalSegmentNum);
TempIctalSegmentNum = TempIctalSegmentNum{1,1};
IctalSegmentNum{1,1} = TempIctalSegmentNum; %%%% For frontal seizures

TempIctalSegmentNum = load('..\..\..\..\03_SeizureDetection\m_TemporalIctalSegmentNum.mat');
TempIctalSegmentNum = struct2cell(TempIctalSegmentNum);
TempIctalSegmentNum = TempIctalSegmentNum{1,1};
IctalSegmentNum{2,1} = TempIctalSegmentNum; %%%% For temporal seizures
clear TempIctalSegmentNum

BestRun = load('m1_BestRun.mat');
BestRun = struct2cell(BestRun);
BestRun = BestRun{1,1};
[NumPatient,NumChanType] = size(BestRun);

FolderNames = dir(); % 获取当前目录列表
FolderNames = FolderNames([FolderNames.isdir]); % 筛选出文件夹
FolderNames = {FolderNames.name}; % 提取文件夹名
FolderNames = setdiff(FolderNames, {'.','..'})'; % 移除特殊目录

%%
Window = 4; %%%%%%%%%%%%%%%%%%%%%%%
Threshold = 3; %%%%%%%%%%%%%%%%%%%%%%%
SegTime = 2; %%%%%% seconds

for ChanTypeNum = 1:NumChanType
    Path = strcat(FolderNames{ChanTypeNum,1},filesep);
    MatInfo = dir(fullfile(Path,'*.mat'));
    MatName = {MatInfo.name}';
    NumPatient = length(MatName);

    for PatientNum = 1:NumPatient
        BestRunIndex = BestRun(PatientNum,ChanTypeNum);
        MatRoute = strcat(Path,MatName{PatientNum,1});
        Labels = load(MatRoute);
        Labels = struct2cell(Labels);
        Labels = cell2mat(Labels);
        [NumSamp,NumType,NumRun] = size(Labels);

        TrueLabels = Labels(:,NumType,BestRunIndex);
        PredLabels = double(Labels(:,2,BestRunIndex) > Labels(:,1,BestRunIndex));

        %%%******* Event-based Sensitivity and Latency for Frontal Seizures *******%%%
        Index0 = [];
        Index0 = find(TrueLabels==0);
        PredLabel0 = [];
        PredLabel0 = PredLabels(Index0);

        Seizures = [];
        Seizures = IctalSegmentNum{1,1}{PatientNum,1};
        NumSeizure = length(Seizures);
        EventSen = [];
        LatencyTime = [];
        for SeizureNum = 1:NumSeizure
            if SeizureNum == 1
                Lstart = 1;
                Lend = sum(Seizures(1:SeizureNum));
            else
                Lstart = sum(Seizures(1:SeizureNum-1))+1;
                Lend = sum(Seizures(1:SeizureNum));
            end

            PredLabel0Select = [];
            PredLabel0Select = PredLabel0(Lstart:Lend);
            count = 0;
            for n = Window:length(PredLabel0Select)
                Lstart = n-Window+1;
                Lend = n;
                Value = sum(PredLabel0Select(Lstart:Lend));
                if Value <= (Window-Threshold)
                    count = count+1;
                end
                if count == 1
                    La = Lend*SegTime;
                end
            end

            if count>0
                EventSen(SeizureNum,1) = 1;
                LatencyTime(SeizureNum,1) = La;
            else
                EventSen(SeizureNum,1) = 0;
                LatencyTime(SeizureNum,1) = NaN;
            end
        end
        Sen = mean(EventSen);
        Latency = mean(LatencyTime,'omitnan');
        TempSenLa(PatientNum,1) = Sen;
        TempSenLa(PatientNum,2) = Latency;

        TempSeizuresCount(PatientNum,1) = sum(EventSen);
        TempSeizuresCount(PatientNum,2) = NumSeizure;


        %%%******* Event-based Sensitivity and Latency for Temporal Seizures *******%%%
        Index1 = [];
        Index1 = find(TrueLabels==1);
        PredLabel1 = [];
        PredLabel1 = PredLabels(Index1);

        Seizures = [];
        Seizures = IctalSegmentNum{2,1}{PatientNum,1};
        NumSeizure = length(Seizures);
        EventSen = [];
        LatencyTime = [];
        for SeizureNum = 1:NumSeizure
            if SeizureNum == 1
                Lstart = 1;
                Lend = sum(Seizures(1:SeizureNum));
            else
                Lstart = sum(Seizures(1:SeizureNum-1))+1;
                Lend = sum(Seizures(1:SeizureNum));
            end

            PredLabel1Select = [];
            PredLabel1Select = PredLabel1(Lstart:Lend);
            count = 0;
            for n = Window:length(PredLabel1Select)
                Lstart = n-Window+1;
                Lend = n;
                Value = sum(PredLabel1Select(Lstart:Lend));
                if Value>=Threshold
                    count = count+1;
                end
                if count == 1
                    La = Lend*SegTime;
                end
            end

            if count>0
                EventSen(SeizureNum,1) = 1;
                LatencyTime(SeizureNum,1) = La;
            else
                EventSen(SeizureNum,1) = 0;
                LatencyTime(SeizureNum,1) = NaN;
            end
        end
        Sen = mean(EventSen);
        Latency = mean(LatencyTime,'omitnan');
        TempSenLa(PatientNum,3) = Sen;
        TempSenLa(PatientNum,4) = Latency;

        TempSeizuresCount(PatientNum,3) = sum(EventSen);
        TempSeizuresCount(PatientNum,4) = NumSeizure;
    end
    SenLa{ChanTypeNum,1} = TempSenLa;
    SeizuresCount{ChanTypeNum,1} = TempSeizuresCount;
end
save('m2_SenLa','SenLa')
save('m2_SeizuresCount','SeizuresCount')

%%
toc
