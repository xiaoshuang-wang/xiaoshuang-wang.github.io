...
for ChanTypeNum = 1:NumChanType
    for PatientNum = 1:NumPatient
        ...
        [NumMetricType,NumRun] = size(TempSSAA);
        TempSSAASelect = [];
        TempSFdLaSelect = [];
        TempSeg = squeeze(TempSSAA(end,:));
        [~, sortedIndices] = sort(TempSeg,'descend');
        top2Indices = sortedIndices(1:2);
        TempSSAASelect = cat(2,TempSSAASelect,TempSSAA(:,top2Indices));
        TempSFdLaSelect = cat(2,TempSFdLaSelect,TempSFdLa(:,:,top2Indices));
...
for ChanTypeNum = 1:NumChanType
    ...
    for PatientNum = 1:NumPatient
        ...
        TempFDR = (TempFDTimes/NumSeizure)/(Interictal(PatientNum)*NumRun);
        Tmp = [];
        TmpLatency = [];
        for RunNum = 1:NumRun
