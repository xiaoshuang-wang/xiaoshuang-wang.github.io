...
SenSpeAcc_SenFpr = load('m0_SSA_SF_ChanVoting.mat');
...
for WindowTypeNum = 1:NumWindowType
    for DatasetNum = 1:NumDataset
        ...
        MeanStd_ChanVoting{WindowTypeNum,DatasetNum} = RunMeanStd;
        clear RunMeanStd
    end
end
save('m1_MeanStd_ChanVoting','MeanStd_ChanVoting')


%% Results of the voted channels are plotted
[NumWindowType,NumDataset] = size(MeanStd_ChanVoting);
for WindowTypeNum = 1%:NumWindowType
    for DatasetNum = 1%:NumDataset
        Data = [];
        Data = MeanStd_ChanVoting{WindowTypeNum,DatasetNum};
