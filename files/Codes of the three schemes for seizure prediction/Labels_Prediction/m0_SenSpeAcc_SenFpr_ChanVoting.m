...
for DatasetNum = 1:NumDataset
    ...
    for LabelMatNum = 1:NumLabelMat
        ...
        TempLabel(:,:,:,1) = ChanVotingLabel;  %%% Weighted voting
        TempLabel(:,:,:,2) = TrueLabel;  %%%

        LabelNew{DatasetNum,1}{LabelMatNum,1} = TempLabel;
        clear ChanVotingLabel TrueLabel TempLabel
    end
end
...
for WindowIndexNum = 1:length(WindowIndex)
    ...
    for DatasetNum = 1:NumDataset
        ...
        for PatientNum = 1:NumPatient
            ...
            ChanVotingLabel = PatientLabel(:,:,:,NumLabelType-1);
            ...
            for RunNum = 1:NumRun
                for SeizureNum = 1:NumSeizure
                    %%**********Segment-based level**********%%
                    TP = sum(ChanVotingLabel(Index1,SeizureNum,RunNum));
                    FN = length(Index1)-TP;
                    FP = sum(ChanVotingLabel(Index0,SeizureNum,RunNum));
                    TN = length(Index0)-FP;
                    ...
                    LabelPred0 = [];
                    LabelPred0 = ChanVotingLabel(Index0,SeizureNum,RunNum);
                    ...
                    LabelPred1 = [];
                    LabelPred1 = ChanVotingLabel(Index1,SeizureNum,RunNum);
...
save('m0_SSA_SF_ChanVoting','SSA_SF')
