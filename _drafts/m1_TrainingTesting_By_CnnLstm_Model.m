...
NewFile = ['Labels_Prediction',filesep];
mkdir(NewFile)
    ...
    for RunNum = 1:NumRun
        for SeizureNum = 1:NumSeizure
            ...
            for ChanNum = 1:NumChan
                ...
                count = count+1;
                TempXTrain = XTrainSelect(ChanNum,:,:);
                XTrain = [];
            ...
            %%%%%*********** training and testing for ALL-CHANNELS*********%%%%%%%%
            count = count+1;
            XTrain = [];
            for SampNum = 1:NumSamp
                XTrain{SampNum,1} = XTrainSelect(:,:,SampNum);
            end
            clear XTrainSelect
            YTrain = categorical(YTrainSelect);
            idx = randperm(NumSamp,floor(NumSamp*ValidationRate));
            ...
            XTest = [];
            for SampNum = 1:size(XTestSelect,3)
            ...
            clear YTrainSelect YTestLabel
        end
    end

    save('m1_TrainNet_AllChan_ChanVoting','TrainNet')
    
    NewRoute = strcat(NewFile,FileName{1,FileNum},'_Labels');
    save(NewRoute,'PredLabel')
    clear NewRoute PredLabel
end
